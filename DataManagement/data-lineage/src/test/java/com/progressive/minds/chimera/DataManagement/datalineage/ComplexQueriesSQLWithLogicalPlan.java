...
public class ComplexQueriesSQLWithLogicalPlan {
    ...
    private static void analyzeLogicalPlan(Dataset<Row> df, String queryName) {
        ...
        // Extract input tables, transformation stages, and input/output columns
        List<String> inputTables = new ArrayList<>();
        List<String> transformationStages = new ArrayList<>();
        extractLogicalPlanDetails(logicalPlan, inputTables, transformationStages);

        // Print results
        System.out.println("Input Tables: " + inputTables);
        System.out.println("Transformation Stages: " + transformationStages);
        System.out.println("----------------------------------------");
    }

    private static void extractLogicalPlanDetails(LogicalPlan plan, List<String> inputTables, List<String> transformationStages) {
        ...
        for (LogicalPlan child : JavaConverters.seqAsJavaList(plan.children())) {
            for (Attribute attribute : JavaConverters.seqAsJavaList(child.output())) {
                inputColumns.add(attribute.name());
            }
        }
        ...
        // Add transformation stage details
        transformationStages.add(
        ...
        for (LogicalPlan child : JavaConverters.seqAsJavaList(plan.children())) {
            extractLogicalPlanDetails(child, inputTables, transformationStages);
        }
    }
}