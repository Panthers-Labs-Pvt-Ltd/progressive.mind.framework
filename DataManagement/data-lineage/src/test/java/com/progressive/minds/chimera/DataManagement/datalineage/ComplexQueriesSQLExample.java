...
public class ComplexQueriesSQLExample {
    private static void extractLogicalPlanDetails(LogicalPlan plan, List<String> inputTables, List<String> transformations) {
        ...
        transformations.add("Transformation: " + plan.nodeName() + ", Output Columns: " + getOutputColumns(plan));

        // Recursively process child plans
        for (LogicalPlan child : JavaConverters.seqAsJavaList(plan.children())) {
            extractLogicalPlanDetails(child, inputTables, transformations);
        }
    }

    private static List<String> getOutputColumns(LogicalPlan plan) {
        List<String> outputColumns = new ArrayList<>();
        for (Attribute attribute : JavaConverters.seqAsJavaList(plan.output())) {
            outputColumns.add(attribute.name());
        }
    ...
    public static void main(String[] args) {
        ...
        SparkSession spark = SparkSession.builder()
                .appName("ComplexQueriesSQLExample")
                .master("local[*]")
                .getOrCreate();
        ...
        totalSalaryByDeptDF.show();

        System.out.println("High Earners (Salary > Average Salary):");
        highEarnersDF.show();

        System.out.println("Projects per Employee:");
        projectsPerEmployeeDF.show();

        System.out.println("Total Compensation (Salary + Bonus):");
        totalCompensationDF.show();
        LogicalPlan logicalPlan = totalCompensationDF.queryExecution().logical();
        List<String> inputTables = new ArrayList<>();
        List<String> transformations = new ArrayList<>();
        extractLogicalPlanDetails(logicalPlan, inputTables, transformations);
        // Stop Spark session
        spark.stop();
    }
}