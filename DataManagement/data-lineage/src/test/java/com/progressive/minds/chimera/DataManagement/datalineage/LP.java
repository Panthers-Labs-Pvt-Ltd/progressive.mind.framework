...
public class LP {
    ...
    public static void parseLogicalPlan(LogicalPlan logicalPlan) {
        ...
        if (logicalPlan instanceof Project) {
            ...
            for (NamedExpression expr : JavaConverters.seqAsJavaList(projectList)) {
                processExpression((Expression) expr, "Project", "Projection");
            }
            ...
            for (NamedExpression expr : JavaConverters.seqAsJavaList(aggregateExpressions)) {
                processExpression((Expression)expr, "Aggregate", "Aggregation");
            }
            ...
            for (NamedExpression expr : JavaConverters.seqAsJavaList(windowExpressions)) {
                processExpression((Expression)expr, "Window", "Window Function");
            }
            ...
            for (SortOrder expr : JavaConverters.seqAsJavaList(scalaSortOrder)) {
                if (expr.child() instanceof NamedExpression) {
                    NamedExpression namedExpr = (NamedExpression) expr.child();
                    transformationDesc = "Sorting " + namedExpr.name() + " By " + expr.direction() + " " + expr.nullOrdering();
                }
                System.out.println(transformationDesc);
                processExpression(expr, "Sort",  transformationDesc);
            }
            parseLogicalPlan(sortPlan.child());
        } else if (logicalPlan instanceof Union) {
            Union unionPlan = (Union) logicalPlan;
            Seq<LogicalPlan> children = unionPlan.children();
            for (LogicalPlan child : JavaConverters.seqAsJavaList(children)) {
    ...
    private static void     processExpression(Expression expr, String transformationType, String transformationDescription) {
        if (expr instanceof Alias) {
            Alias aliasExpr = (Alias) expr;
            Expression childExpr = aliasExpr.child();
            ColumnLineage columnLineage = new ColumnLineage();
            Descendant descendant = new Descendant();
            descendant.origin = "unknown";
            descendant.name = aliasExpr.name();
            columnLineage.descendant = descendant;
            columnLineage.transformationType = transformationType;
            columnLineage.transformationDescription = transformationDescription;

            if (childExpr instanceof Concat) {
                Seq<Expression> children = ((Concat) childExpr).children();
                for (Expression e : JavaConverters.seqAsJavaList(children)) {
                    Lineage lineage = new Lineage();
                    lineage.origin = getSourceTable("");
                    lineage.name = processExpressionName(e);
                    lineage.transformationType = transformationType;
                    lineage.transformationDescription = "Concatenation";
                    columnLineage.lineage.add(lineage);
                }
            } else {
                Lineage lineage = new Lineage();
                lineage.origin = getSourceTable("childExpr");
                lineage.name = processExpressionName(childExpr);
                lineage.transformationType = transformationType;
                lineage.transformationDescription = transformationDescription;
                columnLineage.lineage.add(lineage);
            }
            ...
            processExpression(binaryExpr.left(), "LeftBinary", "LeftBinary");
            processExpression(binaryExpr.right(), "RightBinary", "RightBinary");
        }
        else if (expr instanceof AttributeReference) {
            AttributeReference attributeRef = (AttributeReference) expr;
            String attributeName = attributeRef.name();
            List<String> qualifiers = JavaConverters.seqAsJavaListConverter(attributeRef.qualifier()).asJava();
            String qualifier = qualifiers.isEmpty() ? "N/A" : String.join(".", qualifiers);

            ColumnLineage columnLineage = new ColumnLineage();
            Descendant descendant = new Descendant();
            descendant.origin = qualifier;
            descendant.name = attributeName;
            columnLineage.descendant = descendant;
            Lineage lineage = new Lineage();
            lineage.origin = getSourceTable(qualifier);
            lineage.name = processExpressionName(expr);
            lineage.transformationType = transformationType;
            lineage.transformationDescription = transformationDescription;
            columnLineage.lineage.add(lineage);
            planOutput.columnLineages.add(columnLineage);
    ...
    private static String getSourceTable(String expr) {
    ...
    private static String processExpressionName(Expression expr) {
    ...
    public static List<ColumnLineageUtils.ColumnLineageRecord> convertSeqToList(Seq<ColumnLineageUtils.ColumnLineageRecord> scalaSeq) {
    ...
    public static void main(String[] args) {
        ...
        resultDF.createOrReplaceTempView("flight_data");
        LogicalPlan logicalPlan = spark.sessionState().executePlan(resultDF.queryExecution().analyzed(), CommandExecutionMode.ALL()).optimizedPlan();
