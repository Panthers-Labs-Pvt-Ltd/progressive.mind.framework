...
class RocksDBTypeInfo {
    ...
    private final Map<String, RocksDBTypeInfo.Index> indices;
    ...
    RocksDBTypeInfo(RocksDB db, Class<?> type, byte[] alias) throws Exception {
        ...
        ti.indices().forEach(idx -> {
            ...
            if (idx.parent().isEmpty() || idx.value().equals(KVIndex.NATURAL_INDEX_NAME)) {
                indices.put(idx.value(), new RocksDBTypeInfo.Index(idx, ti.getAccessor(idx.value()), null));
            }
        });
        ti.indices().forEach(idx -> {
            if (!idx.parent().isEmpty() && !idx.value().equals(KVIndex.NATURAL_INDEX_NAME)) {
                indices.put(idx.value(), new RocksDBTypeInfo.Index(idx, ti.getAccessor(idx.value()),
    ...
    RocksDBTypeInfo.Index index(String name) {
        RocksDBTypeInfo.Index i = indices.get(name);
    ...
    class Index {
        ...
        private final RocksDBTypeInfo.Index parent;

        private Index(KVIndex self, KVTypeInfo.Accessor accessor, RocksDBTypeInfo.Index parent) {
