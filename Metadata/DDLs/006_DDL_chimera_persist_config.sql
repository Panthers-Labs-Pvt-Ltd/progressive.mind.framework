CREATE TABLE IF NOT EXISTS chimera_persist_config (
    ...
    CONSTRAINT fk_pipeline_name FOREIGN KEY (pipeline_name) REFERENCES chimera_pipeline (pipeline_name) ON delete CASCADE ON update CASCADE,
    CONSTRAINT fk_data_sink_type FOREIGN KEY (data_sink_type, data_sink_sub_type) REFERENCES chimera_data_sources (data_source_type, data_source_sub_type) ON delete CASCADE ON update CASCADE,
    CONSTRAINT fk_data_source_connection FOREIGN KEY (data_source_connection_name) REFERENCES chimera_data_sources_connections (data_source_connection_name) ON delete CASCADE ON update CASCADE
);
